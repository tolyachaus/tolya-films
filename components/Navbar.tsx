      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 bg-brand-dark/95 backdrop-blur-xl z-50 flex flex-col items-center justify-center"
          >
            <div className="flex flex-col items-center space-y-8 w-full px-6 text-gray-300 text-center">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="text-gray-300 text-3xl font-display font-light tracking-widest hover:text-white transition-colors w-full py-2 drop-shadow-md"
                >
                  {link.name}
                </a>
              ))}
              <div className="flex flex-wrap justify-center gap-4 mt-8 pt-8 border-t border-gray-600/50 w-full max-w-sm">
                <a
                  href={SOCIAL_LINKS.whatsapp}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="WhatsApp"
                  className="w-12 h-12 rounded-full bg-gray-800/50 flex items-center justify-center text-gray-300 hover:text-white hover:bg-gray-700 hover:scale-110 transition-all duration-300 shadow-lg border border-gray-600/30"
                >
